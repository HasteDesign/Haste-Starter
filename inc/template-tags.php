...
if ( ! function_exists( 'haste_starter_the_custom_logo' ) ) {
	...
	function haste_starter_the_custom_logo() {
		...
		if ( function_exists( 'the_custom_logo' ) ) {
			...
			echo has_custom_logo() ? haste_starter_custom_logo_image( $logo ) : haste_starter_custom_logo_name();
		}
		echo '</a>';
	}
}

if ( ! function_exists( 'haste_starter_custom_logo_image' ) ) {
	function haste_starter_custom_logo_image( $logo ) {
		return '<img class="site-logo" alt="Logo ' . esc_attr( get_bloginfo( 'name', 'display' ) ) . '" src="' . esc_url( $logo[0] ) . '">';
